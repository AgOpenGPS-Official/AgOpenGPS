    ...
    public partial class FormFieldExisting : Form
    {
        ...
        private void FormFieldExisting_Load(object sender, EventArgs e)
        {
            ...
            if (dirs == null || dirs.Length < 1)
            {
                ...
                mf.SystemEventsLogger.LogEvent("File Error Load Existing Field");
            ...
            foreach (string dir in dirs)
            {
                ...
                if (File.Exists(filename))
                {
                    using (StreamReader reader = new StreamReader(filename))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            MessageBoxButtons.OK, MessageBoxIcon.Error);
                            mf.SystemEventsLogger.LogEvent(fieldDirectory + " is Damaged, Please Delete,Field.txt is Broken" + ex.ToString());
                            fileList.Add(fieldDirectory);
                            fileList.Add("Error");
                        }
                    }
                ...
                if (File.Exists(filename))
                {
                    ...
                    using (StreamReader reader = new StreamReader(filename))
                    {
                        ...
                        {
                            area = 0;
                            mf.SystemEventsLogger.LogEvent(fieldDirectory + " Boundary.Txt error " + ef.ToString());
                        }
                    }
                    if (area == 0)
                    {
                        fileList.Add("No Bndry");
                        mf.SystemEventsLogger.LogEvent("Boundary is Broken, no Area");
                    }
                ...
                else
                {
                    ...
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                    mf.SystemEventsLogger.LogEvent(fieldDirectory + " is Damaged, Missing Boundary.Txt ");
                }
            ...
            if (fileList == null || fileList.Count < 1)
            {
                ...
                mf.SystemEventsLogger.LogEvent("Create New Field, No Fields Found");
            ...
            {
                mf.TimedMessageBox(2000, gStr.gsNoFieldsFound, gStr.gsCreateNewField);
                mf.SystemEventsLogger.LogEvent("Field Existing, No Fields to List");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            using (StreamReader reader = new StreamReader(fileStr))
            {
                ...
                {
                    mf.SystemEventsLogger.LogEvent("While Opening Field" + ex);
