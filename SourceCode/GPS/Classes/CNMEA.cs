    ...
    public class CNMEA
    {
        ...
        public void SetLocalMetersPerDegree(bool setSim)
        {
            if (setSim && mf.timerSim.Enabled)
            {
                ...
                Properties.Settings.Default.setGPS_SimLatitude = mf.AppModel.latStart;
                Properties.Settings.Default.setGPS_SimLongitude = mf.AppModel.lonStart;
                Properties.Settings.Default.Save();
            }

            mPerDegreeLat = 111132.92 - 559.82 * Math.Cos(2.0 * mf.AppModel.latStart * degreesToRadians) + 1.175
            * Math.Cos(4.0 * mf.AppModel.latStart * degreesToRadians) - 0.0023
            * Math.Cos(6.0 * mf.AppModel.latStart * degreesToRadians);

            mPerDegreeLon = 111412.84 * Math.Cos(mf.AppModel.latStart * degreesToRadians) - 93.5
            * Math.Cos(3.0 * mf.AppModel.latStart * degreesToRadians) + 0.118
            * Math.Cos(5.0 * mf.AppModel.latStart * degreesToRadians);
        ...
        public GeoCoord ConvertWgs84ToGeoCoord(Wgs84 latLon)
        {
            ...
            return new GeoCoord(
                (latLon.Latitude - mf.AppModel.latStart) * mPerDegreeLat,
                (latLon.Longitude - mf.AppModel.lonStart) * mPerDegreeLon);
        }

        public Wgs84 ConvertGeoCoordToWgs84(GeoCoord geoCoord)
        {
            double lat = ((geoCoord.Northing + fixOffset.northing) / mPerDegreeLat) + mf.AppModel.latStart;
            mPerDegreeLon =
                111412.84 * Math.Cos(lat * degreesToRadians)
                - 93.5 * Math.Cos(3.0 * lat * degreesToRadians)
                + 0.118 * Math.Cos(5.0 * lat * degreesToRadians);
            double lon = ((geoCoord.Easting + fixOffset.easting) / mPerDegreeLon) + mf.AppModel.lonStart;
            return new Wgs84(lat, lon);
        }

        public string GetGeoCoordToWgs84_KML(GeoCoord geoCoord)
        {
            double Lat = (geoCoord.Northing / mPerDegreeLat) + mf.AppModel.latStart;
            mPerDegreeLon =
                111412.84 * Math.Cos(Lat * degreesToRadians)
                - 93.5 * Math.Cos(3.0 * Lat * degreesToRadians)
                + 0.118 * Math.Cos(5.0 * Lat * degreesToRadians);
            double Lon = (geoCoord.Easting / mPerDegreeLon) + mf.AppModel.lonStart;
